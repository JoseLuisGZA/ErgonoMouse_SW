...
constexpr bool _hasDuplicateValues(const int *arr, int size, int left = 0, int right = 1) {
  return (left >= size - 1) ? false
         : (right >= size)  ? _hasDuplicateValues(arr, size, left + 1, left + 2)
         : (arr[left] == arr[right]) ? true
                                     : _hasDuplicateValues(arr, size, left, right + 1);
}
static_assert(!_hasDuplicateValues(_keyListCompile, NUMKEYS),
              "KEYLIST assigns more than one button to the same pin in config.h");
