...
/// @brief This function records the minimum and maximum movement of the joysticks: After initialization, move the mouse for 30s and see the printed output.
...
int calcMinMax(int* centered) {    // report internal state as function-result to inform calling loop()
  ...
  if (minMaxCalcState == 0) {
    ...
    Serial.println(F("Start moving the SpaceMouse around for 30s!"));
    ...
    if (millis() - startTime < 30000) {
      ...
      // 30s are over. go to next state and report via console
